...
execute if data entity @n[tag=hygrave.temp.config_checker.text_display] {text: '"a"'} run title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.despawn_time_text.99+_hours.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.despawn_time_text.99+_hours.success",\
  "fallback": "Successfully changed Despawn Time Text / 99+ Hours."\
}

## If success, change value
data modify storage hygrave:common configs.value.graves.text_display.text.despawn_time_text.99+_hours set from storage hygrave:common temp.config.value
