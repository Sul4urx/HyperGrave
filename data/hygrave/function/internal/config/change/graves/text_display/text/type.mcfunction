...
run return run title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.line_$(line_idx).text_$(text_idx).type.fail",\
  "fallback": "§cInvalid type."\
}

$data modify storage hygrave:common configs.value.graves.text_display.text.line_$(line_idx).text_$(text_idx).type set from storage hygrave:common temp.config.value

$title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.line_$(line_idx).text_$(text_idx).type.success",\
