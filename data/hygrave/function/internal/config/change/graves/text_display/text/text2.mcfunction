...
execute if data entity @n[tag=hygrave.temp.config_checker.text_display] {text: '"a"'} run title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.text2.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.text2.success",\
  "fallback": "Successfully changed Text 2."\
}

## If success, change value
data modify storage hygrave:common configs.value.graves.text_display.text.text2 set from storage hygrave:common temp.config.value
