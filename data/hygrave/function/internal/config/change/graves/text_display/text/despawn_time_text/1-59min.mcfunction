...
execute if data entity @n[tag=hygrave.temp.config_checker.text_display] text{text: "a", color: "green"} run title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.despawn_time_text.1-59_minutes.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.despawn_time_text.1-59_minutes.success",\
  "fallback": "Successfully changed Despawn Time Text / 1-59 Minutes."\
}

## If success, change value
data modify storage hygrave:common configs.value.graves.text_display.text.despawn_time_text.1-59_minutes set from storage hygrave:common temp.config.value
