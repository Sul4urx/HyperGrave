...
execute if data entity @n[tag=hygrave.temp.config_checker.text_display] text{text: "a", color: "green"} run title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.text1.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "hygrave.change_config_message.text_display.text.text1.success",\
  "fallback": "Successfully changed Text 1."\
}

## If success, change value
data modify storage hygrave:common configs.value.graves.text_display.text.text1 set from storage hygrave:common temp.config.value
