...
execute unless data storage hygrave:common temp.config.value[] unless data storage hygrave:common temp.config{value:[]} run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_opening_costs.owners.items.change_list.fail.not_a_list",\
...
function hygrave:internal/config/change/costs/grave_opening_costs/owners/items/change_list/check_value with storage hygrave:common temp.args
...
execute if data storage hygrave:common temp.config.value[] unless score .list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_opening_costs.owners.items.change_list.fail.not_valid",\
...
execute if data storage hygrave:common temp.config.value[] if score .list_is_valid hygrave.temp_var matches 1 run title @s actionbar {\
  "translate": "hygrave.change_config_message.grave_opening_costs.owners.items.change_list.success",\
...
execute if data storage hygrave:common temp.config.value[] if score .list_is_valid hygrave.temp_var matches 1 run data modify storage hygrave:common configs.value.costs.grave_opening_costs.owners.items set from storage hygrave:common temp.config.value
execute if data storage hygrave:common temp.config{value:[]} run data modify storage hygrave:common configs.value.costs.grave_opening_costs.owners.items set value []
