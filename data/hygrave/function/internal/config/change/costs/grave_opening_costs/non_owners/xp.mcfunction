...
execute unless predicate {\
  ...
  "translate": "hygrave.change_config_message.grave_opening_costs.non_owners.xp.fail",\
  "fallback": "§cValue must be a non-negative integer."\
}

## Otherwise change values
execute store result score (costs/grave_opening_costs/non_owners/xp) hygrave.config run data get storage hygrave:common temp.config.levels
