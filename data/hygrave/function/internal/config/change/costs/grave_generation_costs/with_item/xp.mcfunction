...
execute unless predicate {\
  ...
  "translate": "hygrave.change_config_message.grave_generation_costs.with_item.xp.fail",\
  "fallback": "§cThe value must be an integer between 0 and 100 (inclusive)."\
}

## Otherwise change values
execute store result score (costs/grave_generation_costs/with_item/xp) hygrave.config run data get storage hygrave:common temp.config.value
