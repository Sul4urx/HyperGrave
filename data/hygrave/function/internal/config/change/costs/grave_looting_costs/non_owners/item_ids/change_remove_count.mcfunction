...
execute unless predicate {\
  ...
  "translate": "hygrave.change_config_message.grave_looting_costs.non_owners.item_remove_count.fail",\
  "fallback": "§cValue must be a non-negative integer."\
}

## Otherwise change values
execute store result score (costs/grave_looting_costs/non_owners/item_remove_count) hygrave.config run data get storage hygrave:common temp.config.value
