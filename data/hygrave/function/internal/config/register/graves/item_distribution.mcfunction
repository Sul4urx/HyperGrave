...
##> Remove
execute unless data storage hygrave:common configs.value.graves.item_distribution.grave_generation_success.remove[] run data modify storage hygrave:common configs.value.graves.item_distribution.grave_generation_success.remove set value [{predicates: {"minecraft:enchantments": [{levels:{enchantments: "minecraft:vanishing_curse"}}]}}]
