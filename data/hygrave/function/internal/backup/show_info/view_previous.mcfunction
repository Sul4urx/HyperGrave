...
## Save BID - 1 in `temp.mcargs.'backup/show_info/view_previous/loop'.bid` and remove 1 from `.bid` temp score
...
execute store result storage hygrave:common temp.mcargs.'backup/show_info/view_previous/loop'.bid int 1 run scoreboard players get .bid hygrave.temp_var
...
execute store result score .return_value hygrave.temp_var run function hygrave:internal/backup/show_info/view_previous/loop with storage hygrave:common temp.mcargs.'backup/show_info/view_previous/loop'

execute store result storage hygrave:common temp.rmcargs.'backup/show_backup_info'.bid int 1 run scoreboard players get .return_value hygrave.temp_var
