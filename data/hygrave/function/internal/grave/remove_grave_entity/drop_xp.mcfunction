...
## Set XP value
data modify entity @n[tag=hygrave.temp.grave.xp_to_drop] Value set from storage hygrave:common graves[-1].drop_queued_contents.xp.after_death.total


## If the XP orb doesn't store any XP, get rid of it
execute store result score .dropped_xp_total hygrave.temp_var run data get storage hygrave:common graves[-1].drop_queued_contents.xp.after_death.total
