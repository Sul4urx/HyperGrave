...
## Items
execute if data storage hygrave:common configs.value.costs.grave_looting_costs.owners.items[] run data modify storage hygrave:common temp.config_copy.items set from storage hygrave:common configs.value.costs.grave_looting_costs.owners.items
data modify storage hygrave:common temp.args.value set from storage hygrave:common temp.config_copy.items[-1]


execute store result score .loop_count hygrave.temp_var if data storage hygrave:common configs.value.costs.grave_looting_costs.owners.items[]
...
execute if score .loop_count hygrave.temp_var matches 1.. run function hygrave:internal/grave/pop/check_costs/owners/items with storage hygrave:common temp.args

execute if score .check_costs.items hygrave.temp_var matches 1 run function hygrave:internal/grave/pop/check_costs/owners/items/remove with storage hygrave:common temp.args.value
