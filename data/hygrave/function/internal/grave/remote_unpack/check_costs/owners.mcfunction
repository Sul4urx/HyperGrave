...
execute if score (costs/grave_remote_looting_costs/owners/gamemodes/survival) hygrave.config matches 0 if entity @s[gamemode=survival] run return run scoreboard players set .check_costs.gamemodes hygrave.temp_var 0
execute if score (costs/grave_remote_looting_costs/owners/gamemodes/adventure) hygrave.config matches 0 if entity @s[gamemode=adventure] run return run scoreboard players set .check_costs.gamemodes hygrave.temp_var 0
...
execute if score .player_xp hygrave.temp_var >= (costs/grave_remote_looting_costs/owners/xp) hygrave.config run scoreboard players set .check_costs.xp hygrave.temp_var 1

execute store result storage hygrave:common temp.args.value int 1 run data get storage hygrave:common configs.value.costs.grave_remote_looting_costs.owners.xp.levels -1
