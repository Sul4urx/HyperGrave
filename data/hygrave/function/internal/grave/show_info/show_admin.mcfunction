...
$execute unless data storage hygrave:common graves[-1].data.status{destroyed:1b} run tellraw @s {\
  ...
  "with": [\
    {\
      ...
      "clickEvent": {\
        "action": "run_command",\
        "value": "/function hygrave:internal/grave/show_info/show_admin/view_previous {gid: $(gid)}"\
      }\
    },\
    {\
      ...
      "clickEvent": {\
        ...
        "value": "/function hygrave:run/grave/admin/remote_open {gid: $(gid)}"\
      }\
    },\
    {\
      "translate": "hygrave.grave_info.menu.back",\
      ...
      "clickEvent": {\
        "action": "run_command",\
        "value": "/function hygrave:internal/grave/show_info/show_admin/view_next {gid: $(gid)}"\
      }\
    }\
  ]\
}
