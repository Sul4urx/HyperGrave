...
## Save GID + 1 in `temp.mcargs.'grave/show_info/show_admin/view_next/loop'.gid` and add 1 to `.gid` temp score
...
execute store result storage hygrave:common temp.mcargs.'grave/show_info/show_admin/view_next/loop'.gid int 1 run scoreboard players get .gid hygrave.temp_var
