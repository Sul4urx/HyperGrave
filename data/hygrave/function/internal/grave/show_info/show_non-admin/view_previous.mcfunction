...
## Save GID - 1 in `temp.mcargs.'grave/show_info/show_non-admin/view_previous/loop'.gid` and remove 1 from `.gid` temp score
...
execute store result storage hygrave:common temp.mcargs.'grave/show_info/show_non-admin/view_previous/loop'.gid int 1 run scoreboard players get .gid hygrave.temp_var

## Save player GID
execute store result storage hygrave:common temp.mcargs.'grave/show_info/show_non-admin/view_previous/loop'.pid int 1 run scoreboard players get @s hygrave.pid
...
execute store result score .return_value hygrave.temp_var run function hygrave:internal/grave/show_info/show_non-admin/view_previous/loop with storage hygrave:common temp.mcargs.'grave/show_info/show_non-admin/view_previous/loop'
