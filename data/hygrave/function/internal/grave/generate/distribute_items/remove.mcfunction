...
execute store result score .loop_count_2 hygrave.temp_var if data storage hygrave:common configs.value.graves.item_distribution.grave_generation_success.remove[]
data modify storage hygrave:common temp.config_copy.item_distribution.remove set from storage hygrave:common configs.value.graves.item_distribution.grave_generation_success.remove
data modify storage hygrave:common temp.args.value set from storage hygrave:common temp.config_copy.item_distribution.remove[-1]
...
execute unless score .slot_operation hygrave.temp_var matches 1.. run function hygrave:internal/grave/generate/distribute_items/remove/check_item_predicates with storage hygrave:common temp.args
