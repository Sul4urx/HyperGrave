...
## Invulnerable Items
execute unless score (dropped_contents/invulnerable_items) sgrave2.config matches 0..1 run scoreboard players set (dropped_contents/invulnerable_items) sgrave2.config 0

execute store result storage sgrave2:common configs.value.dropped_contents.invulnerable_items byte 1 run scoreboard players get (dropped_contents/invulnerable_items) sgrave2.config

execute if data storage sgrave2:common configs.value.dropped_contents{invulnerable_items:0b} run data modify storage sgrave2:common configs.text.dropped_contents.invulnerable_items set value "§c❌"
execute if data storage sgrave2:common configs.value.dropped_contents{invulnerable_items:1b} run data modify storage sgrave2:common configs.text.dropped_contents.invulnerable_items set value "§a✔"

## Invulnerable XP
execute unless score (dropped_contents/invulnerable_xp) sgrave2.config matches 0..1 run scoreboard players set (dropped_contents/invulnerable_xp) sgrave2.config 0

execute store result storage sgrave2:common configs.value.dropped_contents.invulnerable_xp byte 1 run scoreboard players get (dropped_contents/invulnerable_xp) sgrave2.config

execute if data storage sgrave2:common configs.value.dropped_contents{invulnerable_xp:0b} run data modify storage sgrave2:common configs.text.dropped_contents.invulnerable_xp set value "§c❌"
execute if data storage sgrave2:common configs.value.dropped_contents{invulnerable_xp:1b} run data modify storage sgrave2:common configs.text.dropped_contents.invulnerable_xp set value "§a✔"

## No Gravity Items
execute unless score (dropped_contents/no_gravity_items) sgrave2.config matches 0..1 run scoreboard players set (dropped_contents/no_gravity_items) sgrave2.config 0

execute store result storage sgrave2:common configs.value.dropped_contents.no_gravity_items byte 1 run scoreboard players get (dropped_contents/no_gravity_items) sgrave2.config
