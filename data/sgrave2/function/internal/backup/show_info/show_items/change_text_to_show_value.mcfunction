...
$data modify storage sgrave2:common temp.text_to_show.slots.$(slot_id) set value {\
  ...
  "hover_event": {\
    ...
    "components": {\
      "minecraft:lore": [\
        {\
          "translate": "sgrave2.backup_info.items.count", \
          "fallback": "§7Count: %s", \
          "with": [\
            "§a$(count)" \
          ]\
        }, {\
          "translate": "sgrave2.backup_info.items.slot.$(slot_id)", \
          "fallback": "§7Slot: §a$(slot_text)"\
        },"", {\
          "translate": "sgrave2.backup_info.items.click_to_restore", \
  ...
  "click_event": {\
    ...
    "command": "/function sgrave2:run/backup/restore_item {bid: $(bid), slot: $(slot_number)}"\
  }\
}